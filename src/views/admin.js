...
export function admin() {
  ...
  <header class="bg-orange-200 shadow-md">
    <div class="max-w-7xl mx-auto px-8 py-4 flex justify-between items-center">
      ...
      <div class="flex items-center gap-4 max-w-150 min-w-100">
         <button id="btn-add" class="flex-1 capitalize text-amber-50  bg-lime-500 p-2 rounded-lg hover:bg-lime-600 transition">
                add card
         </button>

        <button id="show-users" class="flex-1 capitalize text-amber-50  bg-blue-800 p-2 rounded-lg hover:bg-blue-900 transition">
                show users
         </button>
  ...
  <main class="max-w-7xl mx-auto px-8 py-10">
    ...
    <div id="usersModal" class="fixed inset-0 bg-black/50 flex justify-center items-center hidden">
  <div class="bg-white p-6 rounded-xl w-96 shadow-lg">
    <h2 class="text-xl font-bold mb-4">USERS:</h2>
    <div id="container-users" class="p-5">
      <ul id="list-users"></ul>
       <button
          id = "btnExit"
...
export async function showUsers(){
    ...
    users.forEach((user)=>{
        usersList.innerHTML += `<li>${user.name},${user.role},${user.email},${user.password}, </li> <br>`
    })
}

// funcion para añadir un porducto
export function addCardAdmin (){
